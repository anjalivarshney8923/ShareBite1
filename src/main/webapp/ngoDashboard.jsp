...
<body>
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <p><strong>Phone:</strong> <%= session.getAttribute("phone") != null ? session.getAttribute("phone") : "N/A" %></p>
                                    <p><strong>Address:</strong> <%= session.getAttribute("address") != null ? session.getAttribute("address") : "N/A" %></p>
                                    <p><strong>City:</strong> <%= session.getAttribute("city") != null ? session.getAttribute("city") : "N/A" %></p>
                                    <p><strong>State:</strong> <%= session.getAttribute("state") != null ? session.getAttribute("state") : "N/A" %></p>
                                    <p><strong>ZIP Code:</strong> <%= session.getAttribute("zipCode") != null ? session.getAttribute("zipCode") : "N/A" %></p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
    ...
    <script>
        ...
        function showSection(sectionName) {
            ...
            // Initialize map if showing browse-food section
            if (sectionName === 'browse-food') {
                setTimeout(() => {
                    initMap();
                }, 100);
            }
        ...
        function toggleView(viewType) {
            const listView = document.getElementById('foodListings');
            const mapView = document.getElementById('mapView');
            const listBtn = document.querySelector('button[onclick="toggleView(\'list\')"]');
            const mapBtn = document.querySelector('button[onclick="toggleView(\'map\')"]');
        ...
        function addFoodMarkers() {
            ...
            // Get food listings data (this would typically come from your backend)
            const foodListings = [
                // Sample data - replace with actual data from your backend
                { name: 'Fresh Vegetables', lat: 40.7128, lng: -74.0060, type: 'fresh', city: 'New York' },
                { name: 'Bread and Pastries', lat: 40.7589, lng: -73.9851, type: 'baked', city: 'New York' },
                { name: 'Canned Goods', lat: 40.7505, lng: -73.9934, type: 'canned', city: 'New York' }
            ];
                ...
                const infoWindow = new google.maps.InfoWindow({
                    content: `
                        <div style="padding: 10px;">
                            <h6 style="margin: 0 0 5px 0;">${food.name}</h6>
                            <p style="margin: 0; font-size: 12px;">Type: ${food.type}</p>
                            <p style="margin: 0; font-size: 12px;">City: ${food.city}</p>
                            <button onclick="requestFoodFromMap('${food.name}')" 
        ...
        function getMarkerIcon(foodType) {
            // Return different colored markers based on food type
            const iconColors = {
                'fresh': 'http://maps.google.com/mapfiles/ms/icons/green-dot.png',
                'canned': 'http://maps.google.com/mapfiles/ms/icons/blue-dot.png',
                'baked': 'http://maps.google.com/mapfiles/ms/icons/yellow-dot.png',
                'dairy': 'http://maps.google.com/mapfiles/ms/icons/red-dot.png',
                'frozen': 'http://maps.google.com/mapfiles/ms/icons/purple-dot.png'
            };
            
            return iconColors[foodType] || 'http://maps.google.com/mapfiles/ms/icons/red-dot.png';
        }
        ...
        function requestFoodFromMap(foodName) {
            alert(`Request submitted for ${foodName}! This will open the request form.`);
            // You can implement the actual request functionality here
        }
